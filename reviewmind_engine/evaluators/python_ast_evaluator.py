...
class PythonASTRuleEvaluator(RuleEvaluator):
    def evaluate_file(self, filename: str, content: str, added_lines: set[int], context: dict | None = None) -> list[Finding]:
        ...
        if self.rule.check_pattern:
            ...
            for node in visitor.matches:
                ...
                if line_num in added_lines:
                    line_content = file_lines[line_num - 1] if line_num - 1 < len(file_lines) else ""
                    findings.append(Finding(
                        rule_code=self.rule.rule_code,
                        title=self.rule.title,
                        message=f"Rule Violation (AST): {self.rule.rule_code} - {self.rule.title}\n{self.rule.what_is_wrong}\nCorrect way: {self.rule.what_is_correct}",
                        line=line_num,
                        ...
                        category="security" if "secure" in self.rule.title.lower() or "eval" in self.rule.title.lower() else "style",
