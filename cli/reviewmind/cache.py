...
def flush_violations(token):
    ...
    typer.secho(f"Syncing {len(violations)} offline violations with ReviewMind...", fg=typer.colors.BLUE)
    ...
    if remaining_violations:
        ...
        save_json_file(QUEUE_FILE, queue)
        typer.secho(f"Failed to sync {len(remaining_violations)} violations. Retrying next time.", fg=typer.colors.YELLOW)
    else:
